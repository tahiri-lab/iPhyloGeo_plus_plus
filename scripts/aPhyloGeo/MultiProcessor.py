...
class Multi:
    ...
    It is mainly aimed at very many large tasks that could be run on supercomputers with a ridiculous amount of RAM.
    """
    ...
    def __init__(self, args, function):
        ...
        Args:
            args        (list) The list of value to run.
            One process will be born per first level member
            function (method) The method that needs to run as multiple instance

        The constructor needs two things, a method, and a list of lists.
        Each sub-list of the list will be the list of arguments given to the child process running the given method
        ***The methods used must only take a single list as input;
        and then, deal with its division into other variables***
        ...
        Variables:
            ...
            processlist lists        All the child processes
            ...
            memT        Value(f)    A value accessible from child processes; The total amount of available memory at the creation of the object
             followed   int         The amount of theoretical child processes that can fit in FREE memory
            ...
            amounts into The number of processes that the object will start

            startTime float the present time
            timeForOne Value(f) A value accessible from child processes;
            Time it takes for the first iteration to complete
            rewrite dict ints used the terminal update rewriting process;
            number of lines reminded if large or small
        """
    ...
    def executeOnce(self, arg):
        """
        The method that is run as a single process
    ...
    def processingLargeData(self):
        """
        Method for executing multiprocess on tasks that demand a LARGE amount of individual memory.
        ...
        print("\033[B" * self.rewrite[False],
              flush=True)  # "\033[B" acts as a reverse \n, return the pointer one line up
        ...
        while (len(self.args) != 0):
            if (self.tasks.value < self.maxAllowed.value) & (self.nbAllowed.value >= 1):
                p = Process(target=self.executeOnce,
                            args=([self.args.pop(0)]))  # Multiprocess runs the rest of the processes
        ...
        # These unique prints need to be done because there is no telling where the terminal is at termination time,
        # better make sure it's clean
        print("\r", end="")
        print("\033[B" * self.rewrite[True],
              flush=True)  # "\033[B" acts as a reverse \n, return the pointer one line up
        print("Completed with ", str(self.amount - self.finished.value),
              " errors\n")  # if a process was killed or didn't finish, it will be known here
    ...
    def buttler(self, memBloc):
        ...
        Uses timers to execute its methods because time.sleep() it processor hungry if constantly called
        """
        terminal = time.time()
        mem = time.time()
        while True:
    ...
    def memUpdate(self):
        ...
        This method is run from the buttler() and updates every second

        Variables:
            memBuffer  double %Number of bytes to subtract from the available RAM for safety purposes
        ...
        for child in self.processes:
            try:  # in a try/except because processes ID are never removed from the list
                mem = psutil.Process(child).memory_full_info()[
                    8]  # uss memory usage; hunch much is this process using NOW
                if (self.mem1.value < mem):
                    self.mem1.value = mem  # does it for the whole run in case this maximum is increased by future children
            except:
                ""
    ...
    def terminalUpdate(self, memBlock):
        ...
        This method is run from the battler() and updates every 0.1 second
        """
        ...
        if (memBlock):  # block of prints used only by processingLargeData()
            print("Available memory: ", round(self.memA.value / 10000000) / 100, "/",
                  round(self.memT.value / 10000000) / 100, "Gb        ", end="\n", flush=True)
            print("Active processes: ", str(self.tasks.value) + " / " + str(self.maxAllowed.value) + "            ",
                  end="\n", flush=True)
        ...
        print("\033[A" * self.rewrite[memBlock],
              flush=True)  # "\033[B" acts as a reverse \n, return the pointer one line up
    ...
    def processingSmallData(self):
        """
        Method for executing multiprocess on tasks that demand little to no memory.
        Will immediate start all the child processes
